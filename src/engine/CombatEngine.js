export class CombatEngine {
  ...
  startCombat(enemy, playerStats) {
    this.inCombat = true;
    this.currentEnemy = { ...enemy };
    this.turnCount = 0;
    this.combatLog = [];

    const introMessage = `Combat begins! ${enemy.sounds.appear}`;
  ...
  playerAttack(playerStats) {
    ...
    let message = `You strike the ${this.currentEnemy.name} for ${actualDamage} damage! `;
    message += this.currentEnemy.sounds.hit;
  ...
  enemyAttack(playerStats) {
    ...
    const message = `${this.currentEnemy.sounds.attack} It deals ${actualDamage} damage to you!`;
    this.addToLog(message);
  ...
  playerDefend(playerStats) {
    ...
    const message = 'You take a defensive stance, bracing for impact.';
    this.addToLog(message);

    const baseDamage = this.currentEnemy.attack + Math.floor(Math.random() * 3);
    const reducedDamage = Math.max(0, Math.floor((baseDamage - playerStats.defense) * 0.5));

    const enemyMessage = `${this.currentEnemy.sounds.attack} Your defense absorbs most of the blow! You take ${reducedDamage} damage.`;
    this.addToLog(enemyMessage);

    return {
      success: true,
      enemyDamage: reducedDamage,
      playerHealth: playerStats.health - reducedDamage,
      message: message + ' ' + enemyMessage,
      combatLog: [...this.combatLog],
      combatOver: false
    };
  }
  ...
  playerFlee(playerStats) {
    ...
    if (fled) {
      const message = `You successfully escape from the ${this.currentEnemy.name}!`;
      this.addToLog(message);
      ...
      this.turnCount++;
      const message = `You failed to escape! The ${this.currentEnemy.name} blocks your path.`;
      this.addToLog(message);
  ...
  endCombat(playerVictory) {
    ...
    if (playerVictory) {
      message = `Victory! ${enemy.sounds.death}`;
